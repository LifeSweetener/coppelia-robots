...
def turnLeft(rad):
    ...
    print('Right-Bander2: left ' + str(int(rad*180/math.pi)) + '\n')
...
def turnRight(rad):
    ...
    matrix = sim.rotateAroundAxis(matrix, [0,0,-5], sim.getObjectPosition(robot, -1), rad)      ## rotate {grad} degrees
    sim.setObjectMatrix(robot, -1, matrix)                                                      ## set new robot's orientation (after {grad} degrees rotation)
    print('Right-Bander2: right ' + str(int(rad*180/math.pi)) + '\n')
...
def turnAround():
    ...
    print('Right-Bander2: around 180' + '\n')
...
def actuation():
    global blockTime, Right90Time, flag
    if (sim.getSimulationTime() < blockTime):
        return
    
    if (sim.getSimulationTime() > Right90Time):
        flag = False
    ...
    DISTMAX = 0.2
    DISTMIN = 0.05
    if (distanceRight > DISTMAX) and (not flag):
        turnRight(math.pi/20)
        flag = True
        Right90Time = sim.getSimulationTime() + 1.9
    elif (distanceRight < DISTMIN) and (distanceRight != 0) and (not flag):
        turnLeft(math.pi/20)
        flag = True
        Right90Time = sim.getSimulationTime() + 1.9
    elif (distanceRight == 0) and (not flag):
        turnRight(math.pi/2)
        flag = True
        Right90Time = sim.getSimulationTime() + 2.5
