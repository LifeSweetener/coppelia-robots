...
def sysCall_thread():
    ...
    while not sim.getThreadExistRequest():
        actuation()
```
создаёт отдельный дочерний питоновский процесс для выполнения этого кода. Можно сказать, что это *child threaded script* в терминологии разработчиков CoppeliaSim.
