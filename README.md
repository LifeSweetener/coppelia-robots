...
**Спасибо ребятам (одногруппникам КРМО-01-22 Егору, Лёхе, Ярику и Даниле :boom::star:), сотрудничающим со мной на протяжении выполнения этой работы — без их помощи и поддержки я бы не написал весь этот код!**
...
Кроме того, нужна версия *не ниже 4.3.0* программы!:fire: В этой версии и был добавлен встроенный в редактор Коппелии Питон.
...
def sysCall_thread():
    ...
    while not sim.getThreadExistRequest():
        actuation()
```
создаёт отдельный дочерний питоновский процесс для выполнения всего этого скрипта. Эту функцию и запускает в самом начале главный процесс Коппелии. Можно сказать, что этот скрипт — *child threaded script* в терминологии разработчиков CoppeliaSim.
